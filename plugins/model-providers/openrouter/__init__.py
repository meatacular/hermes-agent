...
class OpenRouterProfile(ProviderProfile):
    ...
    def build_extra_body(self, *, session_id: str | None = None, **context: Any) -> dict[str, Any]:
        ...
        # Ask OpenRouter for usage accounting on every call (2026-09-05). Without it
        # the response carries no ``usage.cost`` or ``cost_details``, so no call can
        # be attributed to the upstream host that served it — the blindness that let
        # one provider overcharge 11x uncaught. Costs no extra tokens or requests and
