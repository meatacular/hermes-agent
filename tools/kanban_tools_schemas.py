...
KANBAN_CREATE_SCHEMA = _schema(
    ...
    ),
    {
        ...
        "max_cost": {
            "type": "number",
            "description": (
                "FLEET: optional per-task cumulative spend cap in USD. Omitted, "
                "the card inherits kanban.default_max_cost from config exactly "
                "as the CLI does; an absent config key leaves it uncapped. When "
                "exceeded, the dispatcher SIGTERMs the worker and blocks the "
                "card with kind=cost_cap (never retried)."
            ),
        },
        "hold": {
            "type": "boolean",
            "description": (
                "FLEET: create the card HELD (blocked / operator_hold). Nothing "
                "runs until a human unblocks it. Charter §5: every job parent "
