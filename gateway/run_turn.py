...
class GatewayTurnMixin:
    ...
    @staticmethod
    async def _await_stream_task(stream_task, stream_consumer: Any = None) -> None:
        """Give the stream consumer task time to flush, then cancel it.

        FLEET: the bound must cover a NATIVE-stream consumer's finalize. WeCom's
        adapter awaits the final-frame ack up to ``_REPLY_ACK_TIMEOUT``, plus up
        to another window draining a pending intermediate ack before the final,
        and synthesises delivery on ack-timeout. A hard 5s cap cancels the
        consumer mid-finalize, which is one of the paths that produced
        "回复了两条". Derive the bound from the adapter's own ack timeout rather
        than hardcoding — finalize = drain (1x) + ack (1x) + margin — so it
        tracks ``_REPLY_ACK_TIMEOUT`` if that changes. Edit/draft consumers keep
        5s: their finalize is a local API round-trip. The happy path returns the
        moment the task completes either way; the trade-off is that a slow WeCom
        ack holds the session busy up to the cap.
        """
        timeout = 5.0
        if stream_consumer is not None and getattr(stream_consumer, "_use_native_streaming", False):
            ack_timeout = 15.0
            with suppress(TypeError, ValueError):
                ack_timeout = float(getattr(
                    getattr(stream_consumer, "adapter", None), "_REPLY_ACK_TIMEOUT", 15.0,
                ))
            timeout = 2.0 * ack_timeout + 4.0
        try:
    ...
    async def _run_agent_mark_streamed_delivery(self, response: Any, turn_ctx: TurnContext) -> None:
        ...
        if not _transformed and (_streamed or _content_delivered):
            ...
            # FLEET duplicate-risk GATE (delivery-boundary dedup). A stream
            # consumer existed but neither suppression flag was observed —
            # either the native-stream finalize ack was still in flight (WeCom:
            # up to a pending-drain plus an ack window, with delivery
            # synthesised on ack-timeout) or the signal was never set. Before
            # running the normal final-send, dedupe on CONTENT: if the consumer
            # has already delivered this exact final text, suppress — it is on
            # screen and a second send duplicates it ("回复了两条"). Exact match
            # only, so genuinely distinct content is never dropped. This covers
            # the residual paths the optimistic pre-ack mark does not: the
            # ``_abandon_native_stream`` cancellation and the best-effort
            # "do not mark" finalize at the dispatch-failure fallback.
            # See website/docs/developer-guide/rca-wecom-stream-final-ack-timeout-duplicate.md.
            from gateway.stream_consumer import should_suppress_duplicate_final_send
            if should_suppress_duplicate_final_send(_sc, _final):
                response["already_sent"] = True
                logger.info(
                    "Suppressed duplicate normal final-send for session %s: stream consumer already "
                    "delivered identical final content (streamed=%s previewed=%s content_delivered=%s "
                    "transformed=%s final_len=%d).",
                    _sk, _streamed, _previewed, _content_delivered, _transformed, len(_final),
                )
            elif getattr(_sc, "stream_deltas_enabled", True):
                # DUPLICATE-RISK DIAGNOSTIC: log the decision inputs
                # ("signal never set" vs "ack-pending race"). Skipped for consumers
                # never fed the final's deltas (interim-only wiring, upstream #105341) —
                # they cannot have raced the normal final send, so the warning would be
