...
class _KanbanDispatcher:
    ...
    # FLEET: a triage task the decomposer repeatedly fails on is a silent stall
    # — it ticks every minute producing nothing, and a spec'd, eligible card can
    # sit unconsumed for an hour (2026-09-04). The first failure of each card is
    # escalated to WARNING, then re-warned on this cadence so an actively stuck
    # card stays loud without spamming every 60s tick.
    DECOMPOSE_FAIL_WARN_INTERVAL = 900

    def __init__(self, kb: Any, settings: _DispatcherSettings) -> None:
        self.kb = kb
        # FLEET: (board, task) -> last WARNING epoch for a stuck decompose.
        self._decompose_fail_warned: dict[tuple[str, str], float] = {}
    ...
    def _decompose_one(self, _decomp: Any, slug: str, tid: str) -> int:
        ...
        if not outcome.ok:
            ...
            # so the immediate tick still logs at debug. FLEET: but a REPEATED
            # failure on the same card is a silent stall, so escalate the first
            # one to WARNING and re-warn on a slow cadence while it stays stuck.
            logger.debug("kanban auto-decompose [%s]: %s skipped: %s", slug, tid, outcome.reason)
            now_ts = time.time()
            key = (slug, tid)
            last_warn = self._decompose_fail_warned.get(key, 0.0)
            if last_warn == 0.0:
                logger.warning(
                    "kanban auto-decompose [%s]: %s failed to decompose: %s",
                    slug, tid, outcome.reason or "(no reason given)",
                )
                self._decompose_fail_warned[key] = now_ts
            elif now_ts - last_warn >= self.DECOMPOSE_FAIL_WARN_INTERVAL:
                logger.warning(
                    "kanban auto-decompose [%s]: %s STILL stuck (%d min): %s",
                    slug, tid, int((now_ts - last_warn) / 60),
                    outcome.reason or "(no reason given)",
                )
                self._decompose_fail_warned[key] = now_ts
            return 0
        # Cleared so a card that succeeds and later re-enters triage gets a
        # fresh WARNING on its next failure.
        self._decompose_fail_warned.pop((slug, tid), None)
