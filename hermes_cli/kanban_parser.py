...
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--max-cost", type=float, default=None, metavar="USD",
             help="FLEET: per-card dollar cap on cumulative worker spend. When the "
                  "card's session costs in state.db exceed it, the dispatcher "
                  "SIGTERMs the worker and BLOCKs the card with kind=cost_cap "
                  "(never retried — routes to the jobsy triage lane). Omit to use "
                  "kanban.default_max_cost from config (absent = uncapped)."),
        _arg("--hold", action="store_true",
             help="FLEET: create the card HELD (blocked / operator_hold): nothing runs "
                  "until a human `hermes kanban unblock`s it. Charter §5 — every job "
                  "parent and every deploy card is created this way."),
    ...
    # FLEET: overwatch cost extension — once per card, bounded by
    # kanban.max_cost_hard_ceiling.
    _cmd("set-cap", [
        _arg("task_id"),
        _arg("cap", type=float, help="new cap in USD (<= kanban.max_cost_hard_ceiling, default 1.50)"),
        _arg("--reason", default="", help="why the extension is justified — recorded on the card"),
    ], help="Raise a card's cost cap ONCE, to at most kanban.max_cost_hard_ceiling"),
    _cmd("request-review", [
        _TASK_ID,
    ...
    _cmd("decompose", _triage_sweep_args("decompose", "Decompose", "decomposer") + (
        _arg("--dry-run", action="store_true",
             help="FLEET: compute the decomposition graph (LLM + routing) and print "
                  "the plan without writing anything to the board. Safe for E2E "
