...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        _print_json({
            **{k: getattr(res, k)
            ...
            # FLEET: cards left gated because a parent was not
            # terminal-successful — skipped this tick, not failed.
            "parent_gated": res.parent_gated,
            "cost_capped": res.cost_capped,
            "frozen_by_circuit": res.frozen_by_circuit,
    ...
    if res.parent_gated:
        print(f"Parent-gated (demoted to todo): {', '.join(res.parent_gated)}")
    if res.cost_capped:
        print(f"Cost-capped (blocked, kind=cost_cap): {', '.join(res.cost_capped)}")
    if res.frozen_by_circuit:
        print("Dispatch PAUSED fleet-wide: provider-outage circuit breaker is open.")
