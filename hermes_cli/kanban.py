...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    # FLEET: explicit --max-cost wins; otherwise fall back to
    # kanban.default_max_cost through the single shared resolver. Absent in
    # both = uncapped (backward compat); a stored NULL is never enforced.
    ...
    if max_cost is not None:
        try:
            ...
            return _err(f"kanban: invalid --max-cost/kanban.default_max_cost {max_cost!r}", 2)
        if max_cost < 0:
...
def _run_triage_sweep(args: argparse.Namespace, verb: str, mod, run_one, json_key: str,
    ...
    # FLEET: --dry-run computes the graph (LLM + routing) and prints the plan
    # without writing to the board, so an E2E check is safe against any board
    # including the live one. Only ``decompose`` accepts it.
    dry_run = bool(getattr(args, "dry_run", False))
    run_kwargs = {"dry_run": True} if dry_run else {}
    ok_count = 0
    for tid in ids:
        outcome = run_one(tid, author=author, **run_kwargs)
        if outcome.ok:
            ok_count += 1
        if want_json:
            _payload = _obj_dict(outcome, json_fields)
            if dry_run:
                _payload["dry_run"] = True
                _payload["plan"] = getattr(outcome, "dry_run_plan", None)
            print(json.dumps(_payload))
        elif outcome.ok and getattr(outcome, "dry_run_plan", None) is not None:
            ...
            for i, c in enumerate(outcome.dry_run_plan):
                ...
                print(f"    {i}. [{c.get('assignee')}] {c.get('title')}"
...
_HANDLERS = {
    # FLEET: "set-cap" is the overwatch cost extension (once per card, bounded
    # by kanban.max_cost_hard_ceiling).
