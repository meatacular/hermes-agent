...
class DispatchResult:
    ...
    spawned — no worker, no ``task_runs`` row. ``recompute_ready`` re-promotes
    once the parent actually reaches a terminal-successful status. Closes the
    spawn-then-block path racy writers (manual unblock, stale-claim re-release,
    a parent reopened after promote) could otherwise open."""
    ...
    """FLEET: task ids whose cumulative worker spend exceeded ``max_cost``.
    SIGTERMed through the shared teardown and blocked with kind ``cost_cap`` —
    never retried; they flow to the jobsy triage lane like other dead letters."""
    ...
    """FLEET: True when this tick spawned nothing because the global dispatch
    circuit breaker is open — a provider-side outage paused dispatch fleet-wide.
    Tasks stay queued; a successful probe closes the breaker and the next tick
    resumes. Distinct from ``memory_pressure`` (host memory, not the provider)."""
...
def enforce_max_runtime(conn: sqlite3.Connection, *, signal_fn=None) -> list[str]:
    ...
    for row in rows:
        ...
        # FLEET: SIGTERM then SIGKILL through the SHARED teardown, so the
        # runtime cap, the cost cap and stale reclaim all kill a worker the same
        # way. Workers wanting a cleaner shutdown install their own handler.
        # Upstream #99558: the teardown carries the spawn fingerprint — a
        # recycled PID is never signalled, and an UNVERIFIED spawn that is still
        # live is neither signalled nor released beside (duplicate); it is
        # reclaimed once it exits.
        termination = _terminate_reclaimed_worker(
            row["worker_pid"], row["claim_lock"], signal_fn=signal_fn,
            started_at=started_at,
        )
        if termination.get("signal_refused") and not termination.get("terminated"):
...
class _DeadWorker:
    ...
    # FLEET: a clean exit caused by a broken workspace is a deliberate
    # ``capability`` block, not a crash — retrying into the same broken
    # workspace only loops, and it must consume neither the failure budget nor
    # the protocol-violation streak.
    workspace_error: bool = False

    @property
    def run_outcome(self) -> str:
        # A rate-limited requeue is recorded as ``rate_limited`` so board history
        # doesn't show a phantom crash for a quota wall; a workspace error is a
        # block, not a crash.
...
def _classify_fleet_clean_exit(
    pid: int, code: Optional[int], claimer: Optional[str], *,
    conn: sqlite3.Connection, task_id: str,
    started_at: Optional[int], board: Optional[str],
) -> Optional[_DeadWorker]:
    """FLEET: classify an rc=0 exit that is NOT a plain protocol violation.

    Provider-outage-at-spawn is checked FIRST: a clean exit inside
    ``CIRCUIT_FAST_EXIT_SECONDS`` with a provider 4xx/5xx in the log is the
    fleet-wide outage signature (2026-08-30: an OpenRouter 403 bounced every
    worker at spawn and the dispatcher retried through 22 crashed runs). It is
    handled like a quota wall — released to the source phase with NO failure
    counted — and it trips the global breaker so the fleet pauses instead of
    retrying every card into the same wall. Otherwise a workspace/worktree
    failure is separated from a genuine ``no_checkpoint`` so the two stop being
    conflated on the board (2026-09-01, t_8fc16a73). Returns ``None`` when
    neither applies, leaving upstream's protocol-violation path to run.
    """
    reaped = _recent_worker_exits.get(int(pid))
    reaped_at = int(reaped[1]) if reaped else None
    if _kb._detect_provider_outage(task_id, started_at, reaped_at, board=board):
        _kb._circuit_trip(
            conn,
        ...
        _kb._queue_circuit_alert(
            conn, "tripped",
            "DISPATCH PAUSED: provider-side outage detected at spawn (worker "
            "exited within 10s, rc=0, provider HTTP 4xx/5xx in log). Global "
            "dispatch circuit breaker opened fleet-wide. A cheap probe will "
            "re-check every few minutes and resume automatically when the "
            "provider is back.",
            board=board,
        )
        return _DeadWorker(
            "clean_exit", code,
            f"pid {pid} exited cleanly (rc=0) with a provider-side HTTP 4xx/5xx "
            f"in the worker log — provider outage detected; released to source "
            f"+ fleet dispatch paused (no failure counted)",
            "provider_outage",
            {"pid": pid, "claimer": claimer, "exit_code": code,
             "provider_outage": True, "violation_class": "provider_error"},
            rate_limited=True,
        )
    if _kb._classify_clean_exit(task_id, board=board) == "workspace_error":
        return _DeadWorker(
            "clean_exit", code,
            "worker exited cleanly (rc=0) without calling kanban_complete or "
            "kanban_block because of a workspace/worktree error in the worker "
            "log — violation_class=workspace_error; blocked as capability so "
            "it is not retried into the same broken workspace",
            "workspace_error",
            {"pid": pid, "claimer": claimer, "exit_code": code,
...
def _classify_dead_worker(
    ...
    protocol violation — ``provider_error`` (fleet-wide outage at spawn),
    ``workspace_error`` (broken worktree) and ``no_checkpoint`` (the genuine
    case). The extra context args are what that split needs; without them the
    classifier behaves exactly as upstream does.

    Upstream: a clean exit or a crash carries the worker's own last output
    (``worker_output`` in the event payload, appended to the error text) so the
    board and the retry worker see WHY instead of a bare label; a rate-limited
    requeue (which includes the fleet's provider-outage release) does not need it.
    """
    ...
    with _kb.write_txn(conn):
        ...
        for row in rows:
            ...
            if dead.workspace_error:
                # FLEET: park it as a typed ``capability`` block instead of
                # releasing it back into the same broken workspace.
                cur = conn.execute(
            ...
            if dead.rate_limited or dead.protocol_violation or dead.workspace_error:
            ...
            if dead.workspace_error:
                # FLEET: a deliberate capability block — neither a crash nor a
                # failure, so it enters no bucket and consumes no budget.
                continue
...
def _dispatch_lane_task(
    ...
    *,
    ...
    if claimed is None:
        return False
    # FLEET: dispatch-time model routing. With no explicit override on the card,
    # the first matching ``kanban.model_rules`` entry sets the worker's model for
    # THIS SPAWN ONLY (never written back). The audit event is how an operator
    # sees why a mechanical card ran on a cheap model.
    fired = _kb.apply_model_rule(claimed, rules=model_rules)
    ...
    # FLEET skill pre-flight (2026-09-05): a skill the assignee cannot load
    # raises ``Unknown skill(s)`` INSIDE the spawned worker at agent init and
    # exits 1, so the card burns its whole retry budget on two identical crashes
    # and the dispatcher only ever sees exit_code 1 — six cards were lost that
    # way. Block once, typed, naming the skill; the escalator routes
    # ``capability`` to whoever can install it. Fail-open by construction.
    missing_skills = _kb.missing_skills_for(claimed.assignee, claimed.skills)
    if missing_skills:
        _skill_dir = (
            "skills" if (claimed.assignee or "default") in ("default", "root")
            else "profiles/" + str(claimed.assignee) + "/skills"
        )
        _kb.block_task(
            conn, claimed.id,
            reason=(
                f"Unknown skill(s) for profile {claimed.assignee!r}: "
                f"{', '.join(missing_skills)}. Blocked BEFORE spawn — the worker "
                f"would have crashed at agent init with exit 1, twice, with no "
                f"diagnosis on the board. Install the skill into "
                f"~/.hermes/{_skill_dir}/ (a profile cannot see root's skills), "
                f"or re-author the card without it."
            ),
            kind="capability",
        )
        result.auto_blocked.append(claimed.id)
...
def _run_reclaim_phase(
    ...
    *,
    ...
    # FLEET: the dollar twin of the runtime cap. A worker can turn over millions
    # of tokens inside one short attempt, which no time cap catches; the ledger
    # read is pure SQL, so enforcement itself spends nothing.
    result.cost_capped = _kb.enforce_max_cost(conn)
...
def _dispatch_once_locked(
    ...
    *,
    ...
    # FLEET: global dispatch circuit breaker. A provider-side outage (an
    # OpenRouter 403/429/5xx at spawn) pauses ALL dispatch — no workers in
    # either lane. Reclaim/promotion above already ran, so cards stay correctly
    # queued; they just wait for a probe to close the breaker. Per-card failure
    # semantics are untouched: the tripping card was already released to its
    # source status with no failure counted.
    if _kb._circuit_open(conn):
        _kb._log.warning(
            "kanban dispatch: circuit breaker open (provider outage); "
            "pause dispatch fleet-wide this tick"
        )
        return _kb._circuit_dispatch_paused(
    ...
    for row in ready_rows:
        ...
        # card in ``ready`` behind an incomplete parent, and spawning it only
        # produces a block-later run. Demote to ``todo`` so ``recompute_ready``
        # re-promotes when the parent actually finishes, and skip this tick
        # entirely — no claim, no worker, no run row. Runs first so
        # ``default_assignee`` auto-assignment also skips a parent-gated card.
        if not _kb._parents_terminal_successful(conn, row["id"]):
            if not dry_run:
                with _kb.write_txn(conn):
                    conn.execute(
