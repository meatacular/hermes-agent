...
def record_response_usage(
    ...
    if agent._session_db and agent.session_id:
        try:
            ...
            # Local import: this module is imported from the transport layer's
            # own call path, so a top-level import would close a cycle.
            from agent.transports.chat_completions import usage_attribution
            agent._session_db.queue_token_counts(
                ...
                model=agent.model,
                api_call_count=1,
                # FLEET: ``response`` here is the RAW SDK ChatCompletion, not a
                # NormalizedResponse — the five attribution fields have to be
                # extracted from the wire object. Reading ``provider_name`` (a
                # NormalizedResponse field) silently returned "" on every call
                # and left 5,387 usage rows with a real cost and no upstream
                # host; ``provider`` is the field OpenRouter actually sends.
                # Verified 2026-09-05 by restarting the gateways and watching
                # it still record nothing.
                **usage_attribution(response),
            )
