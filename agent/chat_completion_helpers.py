    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        for chunk in _iter_provider_stream_chunks(stream, response=lambda: self._attempt_stream_response):
            ...
            if upstream_provider is None:
                _up = getattr(chunk, "provider", None)  # OpenRouter stamps who served
                if not isinstance(_up, str) or not _up:
                    _extra = getattr(chunk, "model_extra", None)
                    _up = _extra.get("provider") if isinstance(_extra, dict) else None
                if isinstance(_up, str) and _up.strip():
                    upstream_provider = _up.strip()
