...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        # This legacy branch runs only when that profile is NOT loaded, and without
        # it those calls would silently keep recording $0 and no upstream provider.
        # (2026-09-05)
        if is_openrouter:
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if hasattr(response, "usage") and response.usage:
            # from_openai fills the three token counts. The aggregator extras —
            # real billed cost, upstream token split, cache discount — are ours,
            # and they are what let a row say WHICH host served the call. Without
            # them every record reads "openrouter" and $0, which is how an
            # upstream provider overcharging 11x went uncaught. (2026-09-05)
            usage = Usage.from_openai(response.usage)
            for _k, _v in _aggregator_usage_extras(response.usage).items():
                setattr(usage, _k, _v)

        # The UPSTREAM host, as the aggregator reports it — never the billing
        # account, which always reads "openrouter". Empty for direct providers.
        provider_name = _aggregator_provider_name(response)
        ...
        return NormalizedResponse(
            ...
            reasoning=getattr(msg, "reasoning", None), usage=usage,
            provider_name=provider_name, provider_data=provider_data or None,
        )
